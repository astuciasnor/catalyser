...
plotar_croqui <- function(delineamento_res, design_type) {
  if (design_type == "split_plot") {
    ...
    blocks <- delineamento_res$blocks
    fator_main_levels <- delineamento_res$fator_main_levels
    fator_sub_levels <- delineamento_res$fator_sub_levels
    a <- length(fator_main_levels)
    s <- length(fator_sub_levels)
    
    # Calcular os índices de coluna e linha dentro de cada bloco
    df$main_plot_idx <- (df$UE - 1) %/% s %% a + 1
    df$sub_plot_idx <- (df$UE - 1) %% s + 1
    
    # Parâmetros de espaçamento e tamanho
    h_sub <- 0.5
    s_sub <- 0.75
    w_col <- 1.9
    w_sub <- 1.6
    
    df$y_offset <- (blocks - df$Bloco) * (s * s_sub + 2.2)
    df$y_header <- df$y_offset + 0.5 + s * s_sub + 0.4
    df$y_sub <- df$y_offset + 0.5 + (s - df$sub_plot_idx) * s_sub + 0.3
    df$x_center <- df$main_plot_idx * 2.4
    
    # Mapear cores para Fator A (Ocean Gradient contrastante: SEAFOAM, BLUE, AMBER)
    main_palette <- c("#62B6B7", "#4D96FF", "#E89B3C", "#E76F51", "#B983FF")
    main_colors <- main_palette[1:min(a, length(main_palette))]
    if (a > length(main_palette)) {
      main_colors <- c(main_colors, grDevices::rainbow(a - length(main_palette)))
    }
    names(main_colors) <- fator_main_levels
    
    # Cores personalizadas para o Fator Sub (Fator B) - fundos coloridos pastel
    sub_fill_palette <- c("#F3E8FF", "#E0F2FE", "#DCFCE7", "#FEF3C7", "#FCE7F3", "#CCFBF1")
    sub_border_palette <- c("#C084FC", "#7DD3FC", "#86EFAC", "#FCD34D", "#FBCFE8", "#5EEAD4")
    sub_text_palette <- c("#6B21A8", "#0369A1", "#15803D", "#B45309", "#BE185D", "#0F766E")
    
    sub_fill_colors <- sub_fill_palette[1:min(s, length(sub_fill_palette))]
    sub_border_colors <- sub_border_palette[1:min(s, length(sub_border_palette))]
    sub_text_colors <- sub_text_palette[1:min(s, length(sub_text_palette))]
    
    if (s > length(sub_fill_palette)) {
      sub_fill_colors <- c(sub_fill_colors, grDevices::rainbow(s - length(sub_fill_palette)))
      sub_border_colors <- c(sub_border_colors, grDevices::rainbow(s - length(sub_border_palette)))
      sub_text_colors <- c(sub_text_colors, rep("#111827", s - length(sub_text_palette)))
    }
    names(sub_fill_colors) <- fator_sub_levels
    names(sub_border_colors) <- fator_sub_levels
    names(sub_text_colors) <- fator_sub_levels
    
    # Injetar cores no data frame das subparcelas
    df$Cor_Sub_Fill <- sub_fill_colors[df$Fator_Sub]
    df$Cor_Sub_Border <- sub_border_colors[df$Fator_Sub]
    df$Cor_Sub_Text <- sub_text_colors[df$Fator_Sub]
    
    # 1. Preparar dados dos containers dos blocos
    block_data <- data.frame()
    for (b in 1:blocks) {
      y_off <- (blocks - b) * (s * s_sub + 2.2)
      y_head <- y_off + 0.5 + s * s_sub + 0.4
      ...
      block_data <- rbind(block_data, data.frame(
        Bloco = b,
    ...
    # 2. Preparar dados dos boxes externos das colunas (Fator A) e seus cabeçalhos
    columns_df <- unique(df[, c("Bloco", "main_plot_idx", "Fator_Main", "y_offset", "y_header", "x_center")])
    columns_df$xmin <- columns_df$x_center - w_col / 2
    columns_df$xmax <- columns_df$x_center + w_col / 2
    columns_df$ymin <- columns_df$y_offset + 0.35
    columns_df$ymax <- columns_df$y_header + 0.4
    columns_df$Cor_Header_Fill <- main_colors[columns_df$Fator_Main]
    columns_df$Cor_Col_Border <- main_colors[columns_df$Fator_Main]
    
    titulo <- "Delineamento em Parcelas Subdivididas"
    subtitulo <- paste("Sorteio Aleat\u00f3rio \u2014 Semente:", delineamento_res$seed)
    
    p <- ggplot() +
      # 1. Containers dos Blocos (borda azulada suave, fundo claro)
      geom_rect(data = block_data, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
                fill = "#FCFCFD", color = "#B0C4DE", linewidth = 0.8) +
      # 2. Aba escura de identificação do Bloco (canto superior esquerdo - metade dentro, metade fora)
      geom_rect(data = block_data, aes(xmin = xmin + 0.1, xmax = xmin + 1.5, ymin = ymax - 0.22, ymax = ymax + 0.22),
                fill = "#0F3B5F", color = NA) +
      geom_text(data = block_data, aes(x = xmin + 0.8, y = ymax, label = label),
                color = "white", fontface = "bold", size = 3.5) +
      # 3. Cabeçalhos das parcelas principais (Preenchimento sólido correspondente)
      geom_rect(data = columns_df, aes(xmin = xmin, xmax = xmax, ymin = y_header - 0.4, ymax = y_header + 0.4,
                                       fill = Cor_Header_Fill), color = NA) +
      # 4. Box externo de cada coluna (Envolve cabeçalho + subparcelas)
      geom_rect(data = columns_df, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax,
                                       color = Cor_Col_Border), fill = NA, linewidth = 0.8) +
      # 5. Rótulo do Fator A no cabeçalho (Texto branco em negrito)
      geom_text(data = columns_df, aes(x = x_center, y = y_header, label = Fator_Main),
                color = "white", fontface = "bold", size = 4.2) +
      # 6. Caixas das subparcelas (Fator B, fundo colorido pastel, borda correspondente)
      geom_rect(data = df, aes(xmin = x_center - w_sub/2, xmax = x_center + w_sub/2,
                               ymin = y_sub - h_sub/2, ymax = y_sub + h_sub/2,
                               fill = Cor_Sub_Fill, color = Cor_Sub_Border), linewidth = 0.5) +
      # 7. Rótulo do Fator B dentro de cada subparcela (Texto colorido em negrito)
      geom_text(data = df, aes(x = x_center, y = y_sub, label = Fator_Sub, color = Cor_Sub_Text),
                fontface = "bold", size = 3.6) +
      # Configurações de escala de identidade e tema
      scale_fill_identity() +
      scale_color_identity() +
      theme_void() +
      labs(title = titulo, subtitle = subtitulo) +
      theme(
        plot.title = element_text(face = "bold", size = 14, color = "#0F3B5F", hjust = 0.5, margin = margin(t = 10, b = 4)),
