...
> **Documento histórico.** Preserva a proposta e a implementação por fases.
> A especificação factual da interface e da exportação atuais permanece no
> arquivo `MODULO_COMUNICACAO_RESULTADOS.md` da raiz.
...
os oito módulos prioritários originais e a ANOVA de um fator, mantendo uma
camada de cálculo separada da camada de apresentação do Word.

Na versão 0.1.5, ANOVA e Gráfico de Linhas usam
`exportacao_codigo_estudo()` como gerador compartilhado da superfície
pedagógica dos scripts numerados e do QMD. O registro técnico continua
preservado, mas separado do código principal que o aluno deve ler.
...
## 5. UI do módulo (quatro sub-abas)

- **1. Esboço do documento:** estrutura prevista, manifesto editorial recolhível
  e seções globais (Introdução, Métodos, Discussão e Conclusão).
- **2. Execuções registradas:** lista ordenável com base, dependência, inclusão
  no Word e componentes disponíveis.
- **3. Bases do projeto:** Base Compartilhada, Bases Derivadas e vínculos com as
  execuções.
- **4. Saída planejada:** conferência de dependências, contadores Word/Projeto R
  e downloads independentes `.docx`/`.zip`.
...
- **Fases 0–3 — concluídas:** registro, contrato, estúdio editorial, QMD
  integrado, namespacing, Word e Projeto R.
- **Fase 4 — incremental:** ampliar e refinar a cobertura conforme cada dupla de
  análises é consolidada, sem reescrever o motor.
- **Fase 5 — futura:** entrosar os geradores canônicos com o livro e avaliar
  quais funções realmente devem migrar para o EAPADados.
...
- **Preparação dos dados:** o Projeto R já captura operações estruturais, Trilha
  compartilhada e receitas dos ramos em scripts ordenados por dependência.
  Preservar essa separação ao refinar novos módulos.
...
`mod_comunicacao.R` usa quatro sub-abas: Esboço do documento, Execuções
registradas, Bases do projeto e Saída planejada. É a ponte visível
**Mouse → Código → Relatório**.
