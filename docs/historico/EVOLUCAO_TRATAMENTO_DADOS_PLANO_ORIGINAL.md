...
> **Documento histórico.** Preserva o plano de evolução por fases e a discussão
> que originou a Trilha e o modelo base/ramos. Não descreve sozinho os locais e
> rótulos da interface atual. A especificação vigente permanece no arquivo
> `EVOLUCAO_TRATAMENTO_DADOS.md` da raiz.
...
> **Adicionar Tratamentos à Base**. Arquivo: `inst/app/modules/mod_tratar.R`
...
```
Liga direto à **regressão logística** (desfecho 0/1) — ponto previsto nas
instruções permanentes do ecossistema para a etapa de preparo.

## 6. UI — a trilha de preparo

O painel atual fica em **Preparando Dados → Adicionar Tratamentos à Base**; o
`mod_tratar` hospeda a Trilha e os tratamentos compartilhados:
...
sobrescrever (como a criação de variáveis já faz); só filtros de linha e recortes são
...
contingência é o exemplo mais claro do conceito de ramo: uma tabela tidy com uma
linha por combinação, coluna `n` e percentual opcional. Ela é específica do
qui-quadrado e nasce da base — logo um `base_contingencia`, nunca a Base
Compartilhada. A etapa **Tabela de Contingência** fica em
**Bases Derivadas → Receita da base** e deve encerrar a receita.

**Decisão de escopo implementada:** a v1 usa uma Base Compartilhada
(`dados_analise`) e vários ramos nomeados `base_<analise>`, todos com um único
salto. Os seletores analíticos oferecem a base compartilhada e somente Bases
Derivadas prontas e atualizadas.
...
**Sinais que a IDE destaca:** a Base Compartilhada é o que as análises leem por
padrão; cada ramo informa origem, nome, finalidade, receita, cache e dimensões.
Filtros, agrupamentos, sumarizações e contingência ficam explicitamente no
módulo Bases Derivadas. A fronteira não é um divisor arrastável na mesma trilha:
ela é representada pelos dois espaços de trabalho, o preparo compartilhado e a
receita da Base Derivada selecionada.

## 7. Plano original em fases

> Esta numeração registra a evolução inicial da Trilha. Ela não deve ser
> confundida com as Fases 3A–3E, adotadas depois para Bases Derivadas,
> execução explícita, Comunicação e exportação.
...
- **Camada legada:** a eventual aposentadoria de `dataset_ativo_rv` deve ser
  gradual, com testes do preparo compartilhado e dos Projetos R exportados.
- **Catálogo futuro:** só ampliar o registro quando o tratamento for frequente,
  dominado e coerente com o escopo da v1. Padronização, classes e duplicatas já
  foram incorporadas; datas e `join` continuam fora do escopo atual.
...
`remover_duplicatas`, `padronizar_texto`. **Agrupar/Sumarizar** também integra a
v1 (uma linha por grupo; `n`, soma, média, mediana, mínimo, máximo e
desvio-padrão), mas como etapa de uma **Base Derivada**, sem substituir a Base
Compartilhada. A arquitetura integrada está em
`PIPELINE_DADOS_E_RELATORIOS.md`.
...
**Feito (Fase 3C.1):** os módulos prioritários originalmente integrados começam como
rascunhos. Resultados só são calculados e mostrados após **Executar análise**.
Uma mudança de base, variável ou opção analítica oculta a prévia anterior e
...
**Feito (piloto 0.1.5):** ANOVA de um fator e Gráfico de Linhas compartilham uma
superfície de código pedagógico entre scripts numerados e QMD. O método
científico aparece em código R legível; a configuração completa permanece em
`metadados/registro_execucoes.rds` para replay.
