...
> **Documento histórico.** Registra a implementação incremental das Fases
> 3A–3E, prompts e decisões intermediárias. Para o fluxo vigente, consulte o
> arquivo `PIPELINE_DADOS_E_RELATORIOS.md` da raiz.
...
| Componente | Estado em 27/07/2026 |
|---|---|
| Importação e visualização | Implementado |
| Pivotar e Separar Dados (empilhar/alargar/separar) | Implementado |
| Organizar Variáveis (criação/arrumação) | Implementado |
| Checagem Final da Base Compartilhada | Implementada em Adicionar Tratamentos à Base |
| Agrupar/sumarizar e contingência | Implementados nas receitas de Bases Derivadas |
...
dados_brutos (originais e imutáveis)
    |
    +-- preparo estrutural compartilhado
    ...
    `-- Trilha de Preparo compartilhada (replay ordenado)
          `-- dados_analise
                ...
                `-- bases derivadas (um salto)
                      +-- base_reg_logistica
                      +-- base_tabela_contingencia
                      +-- base_resumo_ano_especie
                      `-- base_<finalidade>
```
...
### 2. Preparo estrutural compartilhado

Operações que podem mudar a forma ou a unidade de observação da tabela:

- **Pivotar e Separar Dados:** empilhar, alargar e separar;
- **Organizar Variáveis — Criação:** calcular e reescalar, criando colunas;
- **Organizar Variáveis — Arrumação:** renomear, selecionar, remover, tipar,
  recodificar e aplicar outros ajustes leves.

Esses módulos mostram uma prévia, geram código R e só incorporam a mudança
quando o usuário clica em **Adicionar Mudança à Trilha da Base
Compartilhada**. O resultado confirmado compõe `base_resolvida`, sobre a qual a
Trilha é executada.

**Filtrar linhas, Agrupar/Sumarizar e Tabela de Contingência não pertencem a
esta camada.** Como reduzem linhas ou mudam a unidade de observação, são etapas
de receitas de Bases Derivadas.
...
rascunho. Os módulos integrados não calculam nem mostram resultados até
...
O replay integrado cobre os oito módulos prioritários originais das Fases 3B–3C:
...
A ANOVA de um fator foi adicionada como piloto integrado. Na versão 0.1.5, ANOVA
e Gráfico de Linhas também passaram a gerar uma superfície pedagógica humana,
compartilhada entre os scripts numerados e o QMD por
`exportacao_codigo_estudo()`, mantendo os metadados técnicos em seção separada.
...
> visualizar”, “Pivotar e Separar Dados” e “Organizar Variáveis”. Depois mostre
> “Adicionar Tratamentos à Base”, com uma Trilha da Base Compartilhada ordenável
> e reproduzível, contendo tipar, tratar dados faltantes, recodificar,
> dicotomizar, padronizar, criar classes, remover duplicatas e padronizar texto.
> O produto central deve ser “Base Compartilhada — dados_analise”, protegida por
> uma âncora ou cadeado. A partir dela, apresente dois caminhos: análises diretas
> e o botão “Criar base derivada”. Na receita das Bases Derivadas, mostre
> “Filtrar”, “Agrupar/Sumarizar” e “Tabela de Contingência”; em
> Agrupar/Sumarizar, várias linhas viram uma linha por grupo, com o exemplo
> “ano + espécie → n, soma, média e desvio-padrão”. Mostre exemplos em estrela:
> “base_reg_logistica”, “base_tabela_contingencia” e
> “base_resumo_ano_especie”. Deixe explícito: “Use dados_analise por padrão;
> crie uma base derivada somente quando houver preparo específico” e “Ramos
> específicos não alteram a Base Compartilhada”. Não mostre ramos de ramos.
> Inclua a frase “Cada clique gera código R reproduzível”. Estilo acadêmico
